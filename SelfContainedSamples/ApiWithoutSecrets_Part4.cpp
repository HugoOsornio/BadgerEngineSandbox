...
{
	std::string vertexShaderPath(std::string(API_WITHOUT_SECRETS_PART4_CONTENT) + "vert.spv");
	...
	std::string fragmentShaderPath(std::string(API_WITHOUT_SECRETS_PART4_CONTENT) + "frag.spv");
...
void CreateVertexBuffer()
{
  VertexData vertexData[] =
  {
	...
	{
		throw std::runtime_error("Could not bind memory to the vertex buffer");
	}

	void* bufferMemoryPointer;
	if (vkMapMemory(device, vertexBufferMemory, 0, vertexBufferSize, 0, &bufferMemoryPointer) != VK_SUCCESS) 
	{
...
void initWindow()
{
	...
	window = glfwCreateWindow(1920, 1080, "ApiWithoutSecrets_Part4", nullptr, nullptr);
